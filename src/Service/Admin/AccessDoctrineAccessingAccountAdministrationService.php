...
use App\Accessing\Repository\AccessAccountSessionRepository;
...
final class AccessDoctrineAccessingAccountAdministrationService implements AccessAccountAdministrationServiceInterface
{
    ...
    public function __construct(
        ...
        private readonly AccessAccountRepository $accountRepository,
        private readonly AccessAccountSessionRepository $accountSessionRepository,
    ) {
    }
    ...
    private function lockAccount(AccessAccountAdministrationAction $action): void
    {
    ...
    private function unlockAccount(AccessAccountAdministrationAction $action): void
    {
        $account = $this->requireAccount($action->accountReference());
        $account->unlock();
        $this->entityManager->flush();
    ...
    private function terminateSession(AccessAccountAdministrationAction $action): void
    {
    ...
    private function requestSecondFactorReset(AccessAccountAdministrationAction $action): void
    {
        $account = $this->requireAccount($action->accountReference());
