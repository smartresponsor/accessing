...
1. **Two parallel security-event recording mechanisms for the same entity/table** (fragile architecture, also see SOLID/Duplication below):
   - `src/Service/SecurityEvent/AccessSecurityEventService.php` (`AccessSecurityEventServiceInterface`) вЂ” takes strongly-typed `AccessSecurityEventType` + `AccessSecurityEventSeverity` enums.
   - `src/Recorder/SecurityEvent/AccessSecurityEventRecorder.php` (`AccessSecurityEventRecorderInterface`) вЂ” takes a free-form `string $eventType`, no severity, and pulls the current request from `RequestStack` internally rather than accepting it.
...
2. **Dormant/dead legacy 2FA field on `AccessEntity`** (`src/Entity/AccessEntity.php:46,215-221,291-309`): the entity carries its own `$totpSecret` / `setTotpSecret()` / `$secondFactorEnabled` alongside a full relational `AccessSecondFactorEntity` (owned by `AccessSecondFactorService`). `isTotpAuthenticationEnabled()` (line 291) and `getTotpAuthenticationConfiguration()` (line 302) both fall back to the legacy `$totpSecret` field if `secondFactor` is absent. No code in the 130+ files reviewed ever calls `setTotpSecret()` вЂ” it appears to be leftover from a pre-refactor design вЂ” but the fallback logic keeps it alive as a silent, untested code path that Doctrine still persists a column for.
...
4. **Silent-failure DI extension loading** (`src/DependencyInjection/AccessingExtension.php:37-45`): if `config/component/services.yaml` is missing, `load()` just `return`s with no warning/exception вЂ” the bundle would silently register zero services with no diagnostic, making a packaging mistake very hard to detect.

## Bugs & Potential Bugs

1. **CRITICAL вЂ” Obfuscated dynamic property/method call in the password-recovery API path** (`src/Service/Http/Api/Access/AccessApiFlowService.php:397-437`):
   ```php
...
1. **SRP/duplication** вЂ” `AccessSecurityEventService` and `AccessSecurityEventRecorder` (see Architecture #1) are two implementations of "record a security event," with different signatures, doing the same job against the same storage. This is a clear single-responsibility split that should be one abstraction.
...
3. **`AccessRepository::findOneByEmail()` and `findOneByEmailAddress()`** (`src/Repository/AccessRepository.php:41-44,53-63`) are the same lookup with two names вЂ” minor, but adds a second call site to keep in sync if the lookup logic changes.
...
2. **(High)** Fix `AccessEnsureAdminCommand` so it only sets the admin password on first creation (or requires an explicit `--reset-password` flag), and stop using a hardcoded literal (`'admin'`) вЂ” generate a random password and print/store it once, or require an env var.
